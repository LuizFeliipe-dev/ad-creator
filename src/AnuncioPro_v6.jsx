...
const SHOPEE_RATES = {
  ...
  campaignExtraDefault: 0.025,  // +2,5% padrão Campanha Destaque (editável pelo usuário)
  adsFacilDefault:      0.00,   // Ads Fácil: 0–5% editável (vira crédito de Ads)
  fixedFee: {
    cpf:                4.00,
    cpf450:             4.00,
    cnpj:               4.00,
...
//
function getShopeeRates({ sellerType, cpf450, useFreight, useCampaign, campaignRate, useAdsFacil = false, adsFacilRate = 0 }) {
  // campaignRate em decimal, ex: 0.025 para 2,5%
  const effectiveCampaignRate = useCampaign
    ? (campaignRate != null ? campaignRate : SHOPEE_RATES.campaignExtraDefault)
    : 0;
  const effectiveAdsFacilRate = useAdsFacil ? (adsFacilRate || 0) : 0;

  const commissionRate = SHOPEE_RATES.commissionBase
    + (useFreight ? SHOPEE_RATES.freightExtra : 0)
    + effectiveCampaignRate
    + effectiveAdsFacilRate;
  ...
  return {
    ...
    campaignExtra: effectiveCampaignRate,
    adsFacilExtra: effectiveAdsFacilRate,
  };
}
...
function calculatePrice({ cost, margin, marketplace, category, sellerType, cpf450 = false, useFreight = false, useCampaign = false, campaignRate, useAdsFacil = false, adsFacilRate = 0, packagingCost = 0 }) {
  ...
  let commissionRate, fixedFee, paymentFee, freightProgramRate, freightExtra, campaignExtra, adsFacilExtra;

  if (marketplace === "shopee") {
    const rates     = getShopeeRates({ sellerType, cpf450, useFreight, useCampaign, campaignRate, useAdsFacil, adsFacilRate });
    ...
    adsFacilExtra   = rates.adsFacilExtra;
    paymentFee      = 0;
    freightProgramRate = 0;
  } else {
  ...
  const calcBreakdown = (price) => {
    ...
    return {
      ...
      adsFacilAmt:     marketplace === "shopee" ? price * (adsFacilExtra || 0) : 0,
    };
  };
  ...
  const scenarios = [
    calcBreakdown(Math.round(scenarioBase * 0.75 * 100) / 100),
    calcBreakdown(Math.round(scenarioBase * 0.90 * 100) / 100),
    calcBreakdown(Math.round(scenarioBase          * 100) / 100),
  ...
  return {
    ...
    freightExtra, campaignExtra, adsFacilExtra: adsFacilExtra || 0,
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {data.marketplace === "shopee" && data.cost > 0 && (() => {
            ...
            const campaignRateDec = data.useCampaign ? ((data.campaignRate ?? 2.5) / 100) : 0;
            const adsFacilRateDec = data.useAdsFacil ? ((data.adsFacilRate ?? 1) / 100) : 0;
            const rates = getShopeeRates({
              sellerType: st, cpf450: data.cpf450 || false,
              useFreight: data.useFreight || false,
              useCampaign: data.useCampaign || false, campaignRate: campaignRateDec,
              useAdsFacil: data.useAdsFacil || false, adsFacilRate: adsFacilRateDec,
            });
            const totalPct = rates.commissionRate * 100;
            const cols = [
              { label: "Comissão base",   value: "20,0%",  active: true },
              { label: "+ Frete Grátis",  value: data.useFreight  ? "+6,0%"                                      : "—", active: data.useFreight },
              { label: "+ Camp. Destaque",value: data.useCampaign ? `+${(data.campaignRate ?? 2.5).toFixed(1)}%` : "—", active: data.useCampaign },
              { label: "+ Ads Fácil",     value: data.useAdsFacil ? `+${(data.adsFacilRate  ?? 1  ).toFixed(1)}%`: "—", active: data.useAdsFacil },
              { label: "Taxa fixa/item",  value: `R$ ${rates.fixedFee.toFixed(2)}`, active: true },
            ];
            return (
              <div style={{ background: "#fff3f0", border: "1.5px solid #ffc4a8", borderRadius: "var(--radius-sm)", padding: "14px 16px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8 }}>
                  {cols.map(({ label, value, active }) => (
                    <div key={label}>
                      <div style={{ color: "var(--muted)", fontSize: "0.62rem", fontWeight: 700, textTransform: "uppercase" }}>{label}</div>
                      <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "0.95rem", color: active && value !== "—" ? "var(--orange)" : "var(--muted)" }}>{value}</div>
                    </div>
                  ))}
                </div>
          ...
          <div className="breakdown">
            ...
            {result.isShopee ? (<>
              ...
              {result.campaignExtra > 0 && (
                <div className="breakdown-row">
                  <span className="b-label">Campanha Destaque ({fmtPct(result.campaignExtra * 100)})</span>
                  <span className="b-val neg">− {fmtPos(chosenPrice * result.campaignExtra)}</span>
                </div>
              )}
              {result.adsFacilExtra > 0 && (
                <div className="breakdown-row">
                  <span className="b-label">Ads Fácil — crédito Ads ({fmtPct(result.adsFacilExtra * 100)})</span>
                  <span className="b-val neg" style={{ color: "#3b82f6" }}>− {fmtPos(chosenPrice * result.adsFacilExtra)}</span>
                </div>
              )}
            </>) : (<>
              <div className="breakdown-row">

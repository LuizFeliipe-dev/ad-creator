...
//
// ─── SHOPEE 2026 — TABELA DE FAIXAS ─────────────────────────────────────────
// Fonte: seller.shopee.com.br/edu/article/26839
//
// CPF padrão e CNPJ: mesma tabela de faixas por preço do item
// CPF 450+ pedidos/90d: +R$3 na taxa fixa de CADA faixa
//
// Faixa | Preço do item    | Comissão % | Taxa fixa
//   0   | < R$8            | 50%        | (regressiva: = 50% do preço, sem fixa)
//   1   | R$8 – R$79,99    | 20%        | R$4
//   2   | R$80 – R$99,99   | 14%        | R$16
//   3   | R$100 – R$199,99 | 14%        | R$20
//   4   | R$200+           | 14%        | R$26
//
const SHOPEE_TIERS = [
  { maxPrice: 7.99,    commissionRate: 0.50, fixedFee: null,  label: "Até R$7,99" },
  { maxPrice: 79.99,   commissionRate: 0.20, fixedFee: 4.00,  label: "R$8 – R$79,99" },
  { maxPrice: 99.99,   commissionRate: 0.14, fixedFee: 16.00, label: "R$80 – R$99,99" },
  { maxPrice: 199.99,  commissionRate: 0.14, fixedFee: 20.00, label: "R$100 – R$199,99" },
  { maxPrice: Infinity,commissionRate: 0.14, fixedFee: 26.00, label: "R$200+" },
];

const SHOPEE_CPF450_EXTRA = 3.00; // +R$3/item para CPF com 450+/90d

const SHOPEE_EXTRAS = {
  freightExtra:         0.06,
  campaignExtraDefault: 0.025,
};
...
const MARKETPLACES = {
  shopee: {
    ...
    name: "Shopee",
    color: "#EE4D2D",
    commissionRate: 0.20, // base faixa 1 — referência para exibição
    freightExtra:   0.06,
    freightProgram: { label: "Frete Grátis / Shopee Express" },
    notes: "Comissão por faixas de preço (20% até R$79,99 / 14% acima). Taxa fixa varia de R$4 a R$26. CPF 450+/90d: +R$3.",
  },
  mercadolivre: {
    id: "mercadolivre",
    name: "Mercado Livre",
...
const CATEGORIES = [
  "Casa & Decoração", "Eletrônicos", "Moda & Acessórios",
  "Beleza & Cuidados", "Brinquedos & Hobbies", "Esporte & Lazer",
...
function calcShippingAndPackaging({ weightG, length, width, height }) {
  ...
  const pkg      = PACKAGING_COST_TABLE.find(p => billedWeight <= p.maxWeight) || PACKAGING_COST_TABLE[PACKAGING_COST_TABLE.length - 1];
  const shipping = CORREIOS_PAC_TABLE.find(p  => billedWeight <= p.maxWeight)  || CORREIOS_PAC_TABLE[CORREIOS_PAC_TABLE.length - 1];
  return {
    weightG, cubicWeight: Math.round(cubicWeight), billedWeight: Math.round(billedWeight),
    packagingCost: pkg.cost, packagingLabel: pkg.label,
    shippingCost: shipping.baseCost, totalLogisticsCost: pkg.cost + shipping.baseCost,
  };
}

function getShopeeTier(price) {
  return SHOPEE_TIERS.find(t => price <= t.maxPrice) || SHOPEE_TIERS[SHOPEE_TIERS.length - 1];
}

// Retorna comissão %, taxa fixa e detalhes para um preço dado
function getShopeeBreakdown(price, { cpf450, useFreight, useCampaign, campaignRate, useAdsFacil, adsFacilRate }) {
  const tier     = getShopeeTier(price);
  const hasFixed = tier.fixedFee !== null; // false só na faixa < R$8
  const fixedFee = hasFixed
    ? tier.fixedFee + (cpf450 ? SHOPEE_CPF450_EXTRA : 0)
    : 0; // faixa regressiva: comissão já é 50%, sem fixa separada

  const freightExtra  = useFreight  ? SHOPEE_EXTRAS.freightExtra : 0;
  const campaignExtra = useCampaign ? (campaignRate ?? SHOPEE_EXTRAS.campaignExtraDefault) : 0;
  const adsFacilExtra = useAdsFacil ? (adsFacilRate ?? 0) : 0;

  // Extras de % só se aplicam nas faixas com taxa fixa definida
  const totalExtras = hasFixed ? (freightExtra + campaignExtra + adsFacilExtra) : 0;
  const totalRate   = tier.commissionRate + totalExtras;

  return { tier, fixedFee, totalRate, freightExtra, campaignExtra, adsFacilExtra, baseRate: tier.commissionRate };
}

// Iteração numérica para encontrar preço-alvo com faixas dinâmicas
function findShopeePrice(totalFixedCost, targetMarginRate, opts) {
  let price = totalFixedCost / (1 - 0.20 - 0.04 - targetMarginRate); // estimativa inicial
  for (let i = 0; i < 60; i++) {
    const bd      = getShopeeBreakdown(price, opts);
    const newPrice = (totalFixedCost + bd.fixedFee) / (1 - bd.totalRate - targetMarginRate);
    if (Math.abs(newPrice - price) < 0.0005) { price = newPrice; break; }
    price = newPrice;
  }
...
// ─── PRICING CALCULATOR ──────────────────────────────────────────────────────
function calculatePrice({ cost, margin, marketplace, category, sellerType, cpf450 = false, useFreight = false, useCampaign = false, campaignRate, useAdsFacil = false, adsFacilRate = 0, packagingCost = 0 }) {
  ...
  if (marketplace === "shopee") {
    const opts = { cpf450, useFreight, useCampaign, campaignRate, useAdsFacil, adsFacilRate };

    const calcBreakdown = (price) => {
      const bd            = getShopeeBreakdown(price, opts);
      const commissionAmt = price * bd.totalRate;
      const totalDeduct   = commissionAmt + bd.fixedFee + totalFixedCost;
      const profit        = price - totalDeduct;
      const realMargin    = price > 0 ? (profit / price) * 100 : 0;
      return {
        price, commissionAmt, paymentAmt: 0, freightProgAmt: 0,
        fixedFee: bd.fixedFee, cost, packagingCost, totalFixedCost,
        profit, realMargin,
        tier: bd.tier,
        commissionBase:  price * bd.baseRate,
        freightExtraAmt: price * bd.freightExtra,
        campaignAmt:     price * bd.campaignExtra,
        adsFacilAmt:     price * bd.adsFacilExtra,
        totalRate:       bd.totalRate,
        freightExtra:    bd.freightExtra,
        campaignExtra:   bd.campaignExtra,
        adsFacilExtra:   bd.adsFacilExtra,
      };
    };

    const recommended = calcBreakdown(findShopeePrice(totalFixedCost, targetMarginRate, opts));
    const minimum     = calcBreakdown(findShopeePrice(totalFixedCost, 0, opts));
    const base = minimum.price;

    const scenarios = [
      calcBreakdown(Math.round(base * 0.75 * 100) / 100),
      calcBreakdown(Math.round(base * 0.90 * 100) / 100),
      calcBreakdown(Math.round(base              * 100) / 100),
      calcBreakdown(Math.round((base + (recommended.price - base) * 0.33) * 100) / 100),
      calcBreakdown(Math.round((base + (recommended.price - base) * 0.66) * 100) / 100),
      calcBreakdown(Math.round(recommended.price         * 100) / 100),
      calcBreakdown(Math.round(recommended.price * 1.15  * 100) / 100),
      calcBreakdown(Math.round(recommended.price * 1.30  * 100) / 100),
    ];

    const recBd = getShopeeBreakdown(recommended.price, opts);
    return {
      recommended, minimum, scenarios,
      commissionRate: recBd.totalRate,
      fixedFee: recommended.fixedFee,
      freightExtra:   recBd.freightExtra,
      campaignExtra:  recBd.campaignExtra,
      adsFacilExtra:  recBd.adsFacilExtra,
      paymentFee: 0, freightProgramRate: 0, packagingCost,
      isShopee: true,
    };
  }
  ...
  const commissionRate   = mp.categories?.[category]?.[key] || 0.12;
  const fixedFee         = mp.fixedFeeFlat || 0;
  const paymentFee       = mp.paymentFee   || 0;
  const freightProgramRate = useFreight ? (mp.freightProgram?.cost || 0) : 0;
  const totalPercentRate = commissionRate + paymentFee + freightProgramRate;

  const rp = (totalFixedCost + fixedFee) / (1 - totalPercentRate - targetMarginRate);
  const mp_ = (totalFixedCost + fixedFee) / (1 - totalPercentRate);

  const calcBreakdown = (price) => {
    const commissionAmt  = price * commissionRate;
    const paymentAmt     = price * paymentFee;
    const freightProgAmt = price * freightProgramRate;
    const totalDeduct    = commissionAmt + paymentAmt + freightProgAmt + fixedFee + totalFixedCost;
    const profit         = price - totalDeduct;
    const realMargin     = price > 0 ? (profit / price) * 100 : 0;
    return {
      ...
      profit, realMargin,
      commissionBase: commissionAmt, freightExtraAmt: 0,
      campaignAmt: 0, adsFacilAmt: 0, totalRate: commissionRate, tier: null,
    };
  };

  const recommended = calcBreakdown(Math.ceil(rp  * 100) / 100);
  const minimum     = calcBreakdown(Math.ceil(mp_ * 100) / 100);
  const base = minimum.price;
  const scenarios = [
    calcBreakdown(Math.round(base * 0.75 * 100) / 100),
    calcBreakdown(Math.round(base * 0.90 * 100) / 100),
  ...
  return {
    ...
    freightExtra: 0, campaignExtra: 0, adsFacilExtra: 0, isShopee: false,
  };
}

// ─── DIRECT BREAKDOWN (slider em tempo real) ─────────────────────────────────
// Para Shopee: recalcula faixa a cada mudança de preço no slider
function calcBreakdownDirect({ price, cost, packagingCost = 0, isShopee = false, shopeeOpts = null, commissionRate, paymentFee = 0, freightProgramRate = 0, fixedFee }) {
  let commissionAmt, paymentAmt, freightProgAmt, effectiveFixedFee;
  if (isShopee && shopeeOpts) {
    const bd       = getShopeeBreakdown(price, shopeeOpts);
    commissionAmt  = price * bd.totalRate;
    paymentAmt     = 0;
    freightProgAmt = 0;
    effectiveFixedFee = bd.fixedFee;
  } else {
    commissionAmt  = price * commissionRate;
    paymentAmt     = price * paymentFee;
    freightProgAmt = price * freightProgramRate;
    effectiveFixedFee = fixedFee;
  }
  const profit     = price - commissionAmt - paymentAmt - freightProgAmt - effectiveFixedFee - cost - packagingCost;
  const realMargin = price > 0 ? (profit / price) * 100 : 0;
  return { price, commissionAmt, paymentAmt, freightProgAmt, fixedFee: effectiveFixedFee, cost, packagingCost, profit, realMargin };
}
      ...
      {selected && (() => {
        ...
        return (
          <div className="card fade-in">
            ...
            {isShopee ? (
              /* Shopee: faixas por preço */
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
                  {[
                    { label: "Faixa até R$79,99",  value: "20% + R$4" },
                    { label: "R$80 – R$99,99",      value: "14% + R$16" },
                    { label: "R$100 – R$199,99",    value: "14% + R$20" },
                    { label: "R$200+",              value: "14% + R$26" },
                  ].map(({ label, value }) => (
                    <div key={label} className="metric-card">
                      <div className="m-label">{label}</div>
                      <div className="m-value" style={{ fontSize: "0.95rem" }}>{value}</div>
                    </div>
                  ))}
                </div>
                <div style={{ fontSize: "0.74rem", color: "var(--muted)", padding: "6px 2px" }}>
                  <strong>+6%</strong> Frete Grátis/Express &nbsp;·&nbsp; <strong>+2,5%</strong> Campanha Destaque &nbsp;·&nbsp; <strong>+R$3</strong> CPF com 450+/90d
                </div>
              </div>
            ) : (
              /* Outros marketplaces */
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {(data.marketplace === "shopee") && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ border: "1.5px solid var(--border)", borderRadius: "var(--radius-sm)", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: data.useCampaign ? "var(--orange-pale)" : "var(--cream)", gap: 12 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.82rem" }}>Campanha Destaque</div>
                    <div style={{ fontSize: "0.68rem", color: "var(--muted)", marginTop: 1 }}>Taxa adicional cobrada pela Shopee durante o período da campanha — incide sobre todas as vendas da loja</div>
                  </div>
                  <div className="toggle-group" style={{ flexShrink: 0, width: 200 }}>
                    <div className={`toggle-btn ${data.useCampaign ? "active" : ""}`} onClick={() => onChange({ useCampaign: true, campaignRate: data.campaignRate ?? 2.5 })}>
                      Participo
                    </div>
                    <div className={`toggle-btn ${!data.useCampaign ? "active" : ""}`} onClick={() => onChange({ useCampaign: false })}>
                      Não
                    </div>
                  </div>
                </div>
                {data.useCampaign && (
                  <div style={{ padding: "12px 14px", borderTop: "1px solid var(--border)", background: "#fff", display: "flex", alignItems: "center", gap: 12 }}>
                    <div className="form-group" style={{ flex: 1 }}>
                      <label className="form-label">Taxa adicional da campanha (%)</label>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <input
                          className="form-input"
                          type="number" min="0" max="15" step="0.5"
                          style={{ maxWidth: 110 }}
                          value={data.campaignRate ?? 2.5}
                          onChange={e => onChange({ campaignRate: parseFloat(e.target.value) || 0 })}
                        />
                        <span style={{ fontSize: "0.8rem", color: "var(--muted)" }}>% sobre o preço de venda</span>
                      </div>
                      <span className="form-hint">Padrão Shopee: 2,5%. Verifique na Central do Vendedor.</span>
                    </div>
                    <div style={{ background: "var(--orange-pale)", border: "1px solid #ffc4a8", borderRadius: 8, padding: "8px 14px", textAlign: "center", flexShrink: 0 }}>
                      <div style={{ fontSize: "0.62rem", fontWeight: 700, textTransform: "uppercase", color: "var(--orange)" }}>Custo campanha</div>
                      <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "1.1rem", color: "var(--orange)" }}>+{(data.campaignRate ?? 2.5).toFixed(1)}%</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Ads Fácil */}
              <div style={{ border: "1.5px solid var(--border)", borderRadius: "var(--radius-sm)", overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: data.useAdsFacil ? "#eff6ff" : "var(--cream)", gap: 12 }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "0.82rem" }}>Ads Fácil</div>
                    <div style={{ fontSize: "0.68rem", color: "var(--muted)", marginTop: 1 }}>Você define uma % que é descontada de cada venda e vira crédito semanal para Shopee Ads (0% a 5%)</div>
                  </div>
                  <div className="toggle-group" style={{ flexShrink: 0, width: 200 }}>
                    <div className={`toggle-btn ${data.useAdsFacil ? "active" : ""}`}
                      style={data.useAdsFacil ? { background: "#3b82f6", borderColor: "#3b82f6" } : {}}
                      onClick={() => onChange({ useAdsFacil: true, adsFacilRate: data.adsFacilRate ?? 1 })}>
                      Ativo
                    </div>
                    <div className={`toggle-btn ${!data.useAdsFacil ? "active" : ""}`} onClick={() => onChange({ useAdsFacil: false })}>
                      Não
                    </div>
                  </div>
                </div>
                {data.useAdsFacil && (
                  <div style={{ padding: "12px 14px", borderTop: "1px solid var(--border)", background: "#fff", display: "flex", alignItems: "center", gap: 12 }}>
                    <div className="form-group" style={{ flex: 1 }}>
                      <label className="form-label">% de comissão revertida em crédito Ads</label>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <input
                          className="form-input"
                          type="number" min="0" max="5" step="0.5"
                          style={{ maxWidth: 110 }}
                          value={data.adsFacilRate ?? 1}
                          onChange={e => onChange({ adsFacilRate: Math.min(5, parseFloat(e.target.value) || 0) })}
                        />
                        <span style={{ fontSize: "0.8rem", color: "var(--muted)" }}>% (máx. 5%)</span>
                      </div>
                      <span className="form-hint">Esse custo entra no cálculo do preço — o crédito gerado vai para seus Ads.</span>
                    </div>
                    <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "8px 14px", textAlign: "center", flexShrink: 0 }}>
                      <div style={{ fontSize: "0.62rem", fontWeight: 700, textTransform: "uppercase", color: "#3b82f6" }}>Crédito Ads/venda</div>
                      <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "1.1rem", color: "#3b82f6" }}>+{(data.adsFacilRate ?? 1).toFixed(1)}%</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="form-group">
                ...
                <div className="toggle-group">
                  <div className={`toggle-btn ${data.useFreight ? "active" : ""}`} onClick={() => onChange({ useFreight: true })}>✓ Participo (+6%)</div>
                  <div className={`toggle-btn ${!data.useFreight ? "active" : ""}`} onClick={() => onChange({ useFreight: false })}>Não participo</div>
                </div>
                <span className="form-hint">
                  {data.useFreight ? "Adicional de 6% sobre o preço de venda cobrado pela Shopee." : "Frete cobrado do comprador separadamente."}
                </span>
              </div>
          ...
          {data.marketplace === "shopee" && data.cost > 0 && (() => {
            ...
            const opts = {
              cpf450: data.cpf450 || false, useFreight: data.useFreight || false,
              useCampaign: data.useCampaign || false,
              campaignRate: data.useCampaign ? ((data.campaignRate ?? 2.5) / 100) : 0,
              useAdsFacil: data.useAdsFacil || false,
              adsFacilRate: data.useAdsFacil ? ((data.adsFacilRate ?? 1) / 100) : 0,
            };
            // Estima o preço final para determinar a faixa correta
            const estimatedPrice = data.cost * 1.4 + 10; // estimativa conservadora
            const bd = getShopeeBreakdown(estimatedPrice, opts);
            const totalPct = bd.totalRate * 100;
            const cols = [
              { label: "Faixa Shopee",    value: bd.tier.label,                                               active: true },
              { label: "Comissão",        value: fmtPct(bd.baseRate * 100),                                   active: true },
              { label: "+ Frete Grátis",  value: data.useFreight  ? `+${fmtPct(bd.freightExtra  * 100)}` : "—", active: data.useFreight },
              { label: "+ Camp. Destaque",value: data.useCampaign ? `+${fmtPct(bd.campaignExtra * 100)}` : "—", active: data.useCampaign },
              { label: "+ Ads Fácil",     value: data.useAdsFacil ? `+${fmtPct(bd.adsFacilExtra * 100)}` : "—", active: data.useAdsFacil },
              { label: "Taxa fixa/item",  value: `R$ ${bd.fixedFee.toFixed(2)}`,                              active: true },
            ];
            return (
              <div style={{ background: "#fff3f0", border: "1.5px solid #ffc4a8", borderRadius: "var(--radius-sm)", padding: "14px 16px" }}>
                <div style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--orange)", marginBottom: 10 }}>
                  📊 Taxas Shopee ({st}{data.cpf450 && st === "CPF" ? " · 450+ pedidos" : ""})
                  <span style={{ marginLeft: 8, fontWeight: 400, textTransform: "none", color: "var(--muted)" }}>· faixa varia com o preço final</span>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 8 }}>
                  {cols.map(({ label, value, active }) => (
                    <div key={label}>
                      <div style={{ color: "var(--muted)", fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase" }}>{label}</div>
                      <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "0.85rem", color: active && value !== "—" ? "var(--orange)" : "var(--muted)" }}>{value}</div>
                    </div>
                  ))}
                </div>
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid #ffc4a8", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "0.78rem", color: "var(--muted)" }}>Total % sobre o preço de venda (estimado para faixa atual):</span>
...
function StepPricing({ data, onChange, onNext, onBack }) {
  ...
  const result = calculatePrice({
    ...
    useCampaign: data.useCampaign || false,
    campaignRate:data.campaignRate != null ? data.campaignRate / 100 : undefined,
    useAdsFacil: data.useAdsFacil || false,
    adsFacilRate:data.adsFacilRate != null ? data.adsFacilRate / 100 : 0,
    packagingCost,
  });
  ...
  // Limites fixos do slider: mínimo × 0.7 até recomendado × 1.5
  const sliderMin = Math.max(1, Math.floor(min.price * 0.7 * 2) / 2);
  const sliderMax = Math.ceil(rec.price * 1.5 * 2) / 2;

  // Garante que o preço escolhido nunca sai do range
  const rawChosen  = data.chosenPrice != null ? data.chosenPrice : rec.price;
  const chosenPrice = Math.min(Math.max(rawChosen, sliderMin), sliderMax);

  const sliderBreakdown = calcBreakdownDirect({
    price: chosenPrice, cost: data.cost, packagingCost,
    isShopee:   result.isShopee,
    shopeeOpts: result.isShopee ? {
      cpf450:      data.cpf450      || false,
      useFreight:  data.useFreight  || false,
      useCampaign: data.useCampaign || false,
      campaignRate:data.campaignRate != null ? data.campaignRate / 100 : undefined,
    ...
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="form-group" style={{ marginBottom: 14 }}>
            {(() => {
              const sliderMin = Math.max(1, Math.floor(min.price * 0.7 * 2) / 2);
              const sliderMax = Math.ceil(rec.price * 1.5 * 2) / 2; // fixo: 50% acima do recomendado
              const clampedValue = Math.min(Math.max(chosenPrice, sliderMin), sliderMax);
              return (<>
                <input
                  type="range"
                  min={sliderMin}
                ...
                <div className="range-labels">
                  ...
                  <span style={{ color: "var(--muted)" }}>Máx: {fmtPos(sliderMax)}</span>
                </div>
              </>);
            })()}
          </div>
          ...
          {result.isShopee && (() => {
            const bd = getShopeeBreakdown(chosenPrice, {
              cpf450: data.cpf450 || false,
              useFreight: data.useFreight || false,
              useCampaign: data.useCampaign || false,
              campaignRate: data.campaignRate != null ? data.campaignRate / 100 : undefined,
            ...
            return (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", background: "#fff8f0", border: "1px solid #ffd0b0", borderRadius: 6, fontSize: "0.75rem", marginBottom: 2 }}>
                <span>📊</span>
                <span><strong>Faixa Shopee:</strong> {bd.tier.label} — {fmtPct(bd.tier.commissionRate * 100)} + R$ {bd.fixedFee.toFixed(2)}/item{data.cpf450 ? " (+R$3 CPF450)" : ""}</span>
              </div>
            );
          })()}
          <div className="breakdown">
            ...
            {result.isShopee ? (() => {
              ...
              return (<>
                <div className="breakdown-row">
                  <span className="b-label">Comissão Shopee ({fmtPct(bd.baseRate * 100)} — faixa {bd.tier.label})</span>
                  <span className="b-val neg">− {fmtPos(chosenPrice * bd.baseRate)}</span>
                </div>
                {bd.freightExtra > 0 && (
                  <div className="breakdown-row">
                    <span className="b-label">Adicional Frete Grátis / Shopee Express ({fmtPct(bd.freightExtra * 100)})</span>
                    <span className="b-val neg">− {fmtPos(chosenPrice * bd.freightExtra)}</span>
                  </div>
                )}
                {bd.campaignExtra > 0 && (
                  <div className="breakdown-row">
                    <span className="b-label">Campanha Destaque ({fmtPct(bd.campaignExtra * 100)})</span>
                    <span className="b-val neg">− {fmtPos(chosenPrice * bd.campaignExtra)}</span>
                  </div>
                )}
                {bd.adsFacilExtra > 0 && (
                  <div className="breakdown-row">
                    <span className="b-label">Ads Fácil ({fmtPct(bd.adsFacilExtra * 100)})</span>
                    <span className="b-val neg" style={{ color: "#3b82f6" }}>− {fmtPos(chosenPrice * bd.adsFacilExtra)}</span>
                  </div>
                )}
                <div className="breakdown-row">
                  <span className="b-label">Taxa fixa por item{data.cpf450 ? " (R$" + (bd.fixedFee - SHOPEE_CPF450_EXTRA).toFixed(2) + " + R$3 CPF450+)" : ""}</span>
                  <span className="b-val neg">− {fmtPos(bd.fixedFee)}</span>
                </div>
              </>);
            })() : (<>
              <div className="breakdown-row">
      ...
      {data.sellerType === "CNPJ" && data.marketplace === "shopee" && (
        <div className="alert warn">
          <span>⚠️</span>
          <span><strong>CNPJ na Shopee:</strong> A comissão e as taxas fixas são iguais ao CPF (tabela por faixas de preço). Consulte um contador para obrigações fiscais específicas do seu regime tributário.</span>
        </div>
      )}

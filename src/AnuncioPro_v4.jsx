...
// ─── PRICING CALCULATOR ─────────────────────────────────────────────────────
function calculatePrice({ cost, margin, marketplace, category, sellerType, useFreight, packagingCost = 0 }) {
  ...
  // custo total = produto + embalagem (valor fixo, não percentual)
  ...
  const recommendedPrice = totalFixedCost / (1 - totalDeductionRate - targetMarginRate);
  const minPrice = totalFixedCost / (1 - totalDeductionRate);
  ...
  const calcBreakdown = (price) => {
    ...
    const totalDeductions = commission + payment + freight + tax + fixedFee + totalFixedCost;
    const profit = price - totalDeductions;
    const realMargin = (profit / price) * 100;
    return { price, commission, payment, freight, tax, fixedFee, profit, realMargin, cost, packagingCost, totalFixedCost };
  };
  ...
  return { recommended, minimum, scenarios, commissionRate, paymentFee, freightCostRate, taxRate, packagingCost };
}
...
// ─── DIRECT BREAKDOWN ────────────────────────────────────────────────────────
function calcBreakdownDirect({ price, cost, packagingCost = 0, commissionRate, paymentFee, freightCostRate, taxRate, fixedFee }) {
  const commission = price * commissionRate;
  const payment    = price * paymentFee;
  const freight    = price * freightCostRate;
  const tax        = price * taxRate;
  const profit     = price - commission - payment - freight - tax - fixedFee - cost - packagingCost;
  const realMargin = (profit / price) * 100;
  return { price, commission, payment, freight, tax, fixedFee, cost, packagingCost, profit, realMargin };
}

// ─── STEP 3: PRICING ─────────────────────────────────────────────────────────
function StepPricing({ data, onChange, onNext, onBack }) {
  // Resolver custo de embalagem
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="breakdown">
            ...
            {sliderBreakdown.packagingCost > 0 && (
              <div className="breakdown-row">
                <span className="b-label">Custo de embalagem {shipCalc ? `(${shipCalc.packagingLabel})` : ""}</span>
                <span className="b-val neg">− {fmtPos(sliderBreakdown.packagingCost)}</span>
              </div>
            )}

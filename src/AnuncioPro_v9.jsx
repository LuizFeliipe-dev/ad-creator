...
// Fonte: seller.shopee.com.br/edu/article/26839 (vigente março/2026)
//
// CPF padrão, CPF 450+ e CNPJ: MESMA tabela de faixas
// CPF 450+ pedidos/90d: +R$3 na taxa fixa de CADA faixa
//
// Frete Grátis: OBRIGATÓRIO para todos desde mar/2026 — já embutido na comissão
// Subsídio Pix (5% ou 8%): bancado pela Shopee, sem custo para o vendedor
//
// Faixa | Preço do item      | Comissão % | Taxa fixa
//   0   | < R$8              | ~50%       | regressiva (metade do preço do produto)
//   1   | R$8 – R$79,99      | 20%        | R$4
//   2   | R$80 – R$99,99     | 14%        | R$16
//   3   | R$100 – R$199,99   | 14%        | R$20
//   4   | R$200 – R$499,99   | 14%        | R$26
//   5   | R$500+             | 14%        | R$26
//
const SHOPEE_TIERS = [
  { maxPrice: 7.99,    commissionRate: 0.50, fixedFee: null,  label: "Até R$7,99 (regressiva)" },
  ...
  { maxPrice: 499.99,  commissionRate: 0.14, fixedFee: 26.00, label: "R$200 – R$499,99" },
  { maxPrice: Infinity,commissionRate: 0.14, fixedFee: 26.00, label: "R$500+" },
];

const SHOPEE_CPF450_EXTRA = 3.00; // +R$3/item para CPF com 450+/90d

const SHOPEE_EXTRAS = {
  // Frete Grátis obrigatório desde mar/2026 — embutido na comissão, sem custo adicional
...
const MARKETPLACES = {
  shopee: {
    ...
    notes: "Comissão por faixas de preço: 20%+R$4 até R$79,99 / 14%+R$16–R$26 acima. Frete Grátis obrigatório (embutido). CPF 450+/90d: +R$3/item.",
  },
  mercadolivre: {
    id: "mercadolivre",
    name: "Mercado Livre",
...
function getShopeeBreakdown(price, { cpf450, useCampaign, campaignRate, useAdsFacil, adsFacilRate }) {
  const tier     = getShopeeTier(price);
  const hasFixed = tier.fixedFee !== null;
  const fixedFee = hasFixed ? tier.fixedFee + (cpf450 ? SHOPEE_CPF450_EXTRA : 0) : 0;

  // Frete Grátis obrigatório e embutido — não há percentual adicional
  ...
  const totalExtras   = hasFixed ? (campaignExtra + adsFacilExtra) : 0;
  const totalRate     = tier.commissionRate + totalExtras;

  return { tier, fixedFee, totalRate, freightExtra: 0, campaignExtra, adsFacilExtra, baseRate: tier.commissionRate };
}
...
function calculatePrice({ cost, margin, marketplace, category, sellerType, cpf450 = false, useFreight = false, useCampaign = false, campaignRate, useAdsFacil = false, adsFacilRate = 0, packagingCost = 0 }) {
  ...
  if (marketplace === "shopee") {
    const opts = { cpf450, useCampaign, campaignRate, useAdsFacil, adsFacilRate };
            ...
            {isShopee ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ fontSize: "0.74rem", color: "var(--muted)", padding: "6px 2px" }}>
                  🚚 Frete Grátis obrigatório (sem custo adicional) &nbsp;·&nbsp; <strong>+2,5%</strong> Campanha Destaque &nbsp;·&nbsp; <strong>+R$3</strong> CPF 450+/90d
                </div>
              </div>
            ) : (
              /* Outros marketplaces */
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {(data.marketplace === "shopee") && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {/* Frete Grátis: obrigatório desde mar/2026 — informativo apenas */}
              <div className="alert info" style={{ marginTop: 4 }}>
                <span>🚚</span>
                <span><strong>Frete Grátis obrigatório</strong> desde março/2026. O custo do frete é bancado pela Shopee via subsídio — não há percentual adicional na sua comissão.</span>
              </div>
          ...
          {data.marketplace === "shopee" && data.cost > 0 && (() => {
            ...
            const cols = [
              ...
              { label: "Frete Grátis",    value: "Incluído ✓",                                                   active: true },
          ...
          <div className="breakdown">
            ...
            {result.isShopee ? (() => {
              ...
              return (<>
                ...
                {/* Frete Grátis embutido na comissão base — sem linha separada */}
                {bd.campaignExtra > 0 && (
                  <div className="breakdown-row">
